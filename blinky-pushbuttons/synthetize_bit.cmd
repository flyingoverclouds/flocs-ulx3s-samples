...
DEL blinky_pushbutton_yosys.json blinky_pushbutton_ulx3s%BOARDVARIANT%_out.config blinky_pushbutton_ulx3s%BOARDVARIANT%.bit abc.history

REM - yosys will generate the blinky.json file (netlist)
yosys blinky_pushbutton.ys
...
nextpnr-ecp5 --%BOARDVARIANT% --json blinky_pushbutton_yosys.json --lpf ulx3s_v20.lpf --textcfg blinky_pushbutton_ulx3s%BOARDVARIANT%_out.config 

REM - ecppack will generate the binary BIT for the ECP5 fpga
ecppack blinky_pushbutton_ulx3s%BOARDVARIANT%_out.config blinky_pushbutton_ulx3s%BOARDVARIANT%.bit
